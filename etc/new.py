...
def detect_log_type(log_file):
    ...
    # Check if the first line matches the Linux log format without regex
    # Linux log format: "Jun  9 06:06:20 combo syslogd 1.4.1: restart."
    # Checking for a 3-letter month, day (1 or 2 digits), time (hh:mm:ss)
    ...
    # Check if the first line is a sequence of dashes or only digits (used for BGL logs)
    # Example: logs that start with "-" or numeric sequences.
    elif first_line.startswith("-") or first_line.strip().isdigit():
        print("Matched BGL log pattern")
        return "bgl"
    
    # For HDFS logs, use regex to match patterns typical in Hadoop-related logs
    # Example format: "081109 203518 143 INFO dfs.DataNode$DataXceiver: Receiving block ..."
    elif re.match(r'^\d{6}\s+\d{6}\s+\d+\s+[A-Z]+\s+\S+:.*', first_line):
        print("Matched HDFS log pattern")
        return "hdfs"
    
    # Default to 'linux' for any other type (this is the fallback case)
